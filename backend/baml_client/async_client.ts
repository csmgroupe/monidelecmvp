...
export class BamlAsyncClient {
  ...
  async AnalyzeMultipleFloorPlans(
      floorplans: Image[],
    ...
    try {
      ...
      return raw.parsed(false) as FloorPlanAnalysis
    } catch (error) {
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.FloorPlanAnalysis, FloorPlanAnalysis> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      const env = options.env ? { ...process.env, ...options.env } : { ...process.env };
      ...
      return new BamlStream<partial_types.FloorPlanAnalysis, FloorPlanAnalysis>(
        raw,
        (a): partial_types.FloorPlanAnalysis => a,
        (a): FloorPlanAnalysis => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
      throw toBamlError(error);
